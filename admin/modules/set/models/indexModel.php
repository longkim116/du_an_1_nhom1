...
function  update_widget($data_widget, $id)
{
    db_update("interface_block", $data_widget, "`widget_id`='{$id}'");
}

function get_widget_by_id($id)
{
    $sql = db_fetch_row("SELECT * FROM `interface_block` WHERE `widget_id`= '{$id}' ");
    return $sql;
}
